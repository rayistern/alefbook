...
/**
 * Children's Haggadah template — same body as adult, different preamble
 * (kid-friendly fonts, colors, ornaments). source.tex is generated by
 * scripts/sync-standalone.sh from the kids preamble + adult body.
 */
function haggadahKidsTemplate(): TemplateFiles {
  const sourcePath = path.join(process.cwd(), 'templates/haggadah-kids-latex/source.tex')
  const main = readFileSync(sourcePath, 'utf-8')

  const seen = new Set<string>()
  const images: TemplateImage[] = []
  for (const dir of ['templates/haggadah-kids-images']) {
    const dirPath = path.join(process.cwd(), dir)
    try {
      const files = readdirSync(dirPath)
      for (const file of files) {
